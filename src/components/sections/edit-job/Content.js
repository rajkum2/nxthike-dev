...
export default function Content({ data }) {
  ...
  const [category, setCategory] = useState(data.cat_id ? data.cat_id : "");
  const [subCat, setSubCat] = useState(data.sub_cat_id ? data.sub_cat_id : "");
  const [comp_name, setCompName] = useState(
    data.company_name ? data.company_name : ""
  );
  const [loc, setLoc] = useState(
    data.item_location_id ? data.item_location_id : ""
  );
  const [experience, setExp] = useState(
    data.item_experience_id ? data.item_experience_id : ""
  );
  const [salary, setSalary] = useState(data.salary ? data.salary : 0);
  const [role, setRole] = useState(data.role ? data.role : "");
  const [jobType, setJobType] = useState(
    data.item_job_type_id ? data.item_job_type_id : ""
  );
  const [industry, setIndustry] = useState(
    data.industry_type ? data.industry_type : ""
  );
  const [skills, setSkills] = useState(data.key_skills ? data.key_skills : "");
  ...
  const [count, setCount] = useState(
    data.applicants_no ? data.applicants_no : 0
  );
                  ...
                  <div className="col-lg-6">
                    <div className="form-group">
                      ...
                      <Select
                        ...
                        value={
                          category === "cat0644d4cddaf9441f8e600804d47de47c"
                            ? options.subCategory.design.filter(
                                (sub_cat) => subCat === sub_cat.value
                              )
                            : category === "cat8f81bcecbd391456b31387589ae3771f"
                            ? options.subCategory.development.filter(
                                (sub_cat) => subCat === sub_cat.value
                              )
                            : category === "cat85f22c4a468aab13b4af012e0c9f87a2"
                            ? options.subCategory.business.filter(
                                (sub_cat) => subCat === sub_cat.value
                              )
                            : null
                        }
                  ...
                  <div className="col-lg-6">
                    <div className="form-group">
                      ...
                      <Select
                        ...
                        value={options.exp.filter(
                          (exp) => experience === exp.value
                        )}
                      />
                    </div>
                  </div>
                    ...
                    <div className="form-group">
                      ...
                      <div className="smm_input">
                        <Select
                          ...
                          value={options.jobType.filter(
                            (type) => jobType === type.value
                          )}
                        />
                      </div>
                    </div>
